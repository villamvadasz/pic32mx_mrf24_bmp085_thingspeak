...
// Implement consistent ultoa_my() function
...
void ultoa_my(uint32_t Value, uint8_t * Buffer);
#else
// HI-TECH PICC-18 PRO 9.63, C30 v3.25+, and C32 v1.12+ already have a ultoa_my() stdlib
// library function, but it requires 3 parameters.  The TCP/IP Stack
// assumes the C18 style 2 parameter ultoa_my() function, so we shall
// create a macro to automatically convert the code.
#include <stdlib.h>
#define ultoa_my(val,buf)  ultoa_my((char *)(buf),(val),10)
#endif
...
void uitoa_my(uint16_t Value, uint8_t * Buffer);
