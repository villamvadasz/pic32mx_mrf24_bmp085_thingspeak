...
#ifdef STACK_USE_UART_SNIFF_SNTP
    #include "rtc.h"
#endif
...
void SNTPClient(void)
{
    ...
    case SM_UDP_RECV:
...
#ifdef STACK_USE_UART_SNIFF_SNTP
        {
            ...
            char tempBuffer[128];
            putrsUART("Time Received:\r\n");
            ntp_seconds = SNTPGetUTCSeconds();
            rtc_CalculateNtpToDate(ntp_seconds, &timeDate);
            sprintf(tempBuffer, (const char *)"%4.4i.%2.2i.%2.2i\r\n", (int)timeDate.year, (int)timeDate.month, (int)timeDate.day);
            putsUART(tempBuffer);
			sprintf(tempBuffer, (const char *)"%2.2i:%2.2i:%2.2i\r\n", (int)timeDate.hour, (int)timeDate.min, (int)timeDate.sec);
            putsUART(tempBuffer);
        }
#endif
        break;
